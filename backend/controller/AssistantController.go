...
func BookAssistant(c *gin.Context) {
    ...
    var count int64
    db.Model(&models.AssistantBooking{}).Where("assistant_id = ? AND date = ?", request.AssistantID, combinedDateTime).Count(&count)
    if count > 0 {
        c.JSON(http.StatusConflict, gin.H{"error": "Assistant is not available on the selected date"})
        return
    }
